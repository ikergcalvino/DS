...
public class MatrixFunctions {    
    ...
    public static int[] allColumnSums(int[][] a){
        int numColumnas = 0;
        for (int i = 0; i < a.length; i++) {
            if (a[i].length > numColumnas){numColumnas = a[i].length;}
        }
     ...
     public static boolean isMagic(int[][] a) {
         ...
         int d1 = 0;//sumas de las diagonales
         int d2 = 0;
         int columnas[];//suma de las columnas
         ...
         if (isSquare(a) && isRowMagic(a) && isColumnMagic(a)) {
             ...
             for (int i = 0; i < a.length; i++) {//sumar diagonales
                 for (int j = 0; j < a[i].length; j++) {
                     if ((i+j) == (a.length-1)) {d2 += a[i][j];} 
                     if (i == j) {d1 += a[i][j];}
                 }
                 
             }
             
             if ((d1 == d2) && (d2 == columnSum(a,0)) && (rowSum(a,0)==columnSum(a,0))) {//comparar diagonales y una columna
                 resultado = true;               
             }   
     ...
     public static boolean isSequence(int[][] a) {//FALLA CON DESORDENADA
         if (isSquare(a)) {
            int rowSum[];
            int sum = 0;
            int sum2 = 0;
